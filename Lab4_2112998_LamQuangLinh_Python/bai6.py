...
wb = load_workbook('thongtinsinhvien.xlsx')
...
if __name__ == "__main__":
    ...
    heading = Label(root, text="THÔNG TIN ĐĂNG KÝ HỌC PHẦN", bg="light green" font="Time New RoMan")
    ...
    for i, subject in enumerate(subjects):
        Checkbutton(root, text=subject, variable=subject_vars[i]).grid(row=i+10, column=1, sticky=W)
    
    subjects = ["Lập trình java"]
    subject_vars = [IntVar() for _ in subjects]
    
    for i, subject in enumerate(subjects):
        Checkbutton(root, text=subject, variable=subject_vars[i]).grid(row=i+11,column=1, sticky=W)
    
    subjects = ["Công nghệ phần mềm"]
    subject_vars = [IntVar() for _ in subjects]
    
    for i, subject in enumerate(subjects):
        Checkbutton(root, text=subject, variable=subject_vars[i]).grid(row=i+10,column=2, sticky=W)
    
    subjects = ["Phát triển ứng dụng wed"]
    subject_vars = [IntVar() for _ in subjects]
    
    for i, subject in enumerate(subjects):
        Checkbutton(root, text=subject, variable=subject_vars[i]).grid(row=i+11,column=2, sticky=W)
